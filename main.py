...
def reports_to_csv(report_file, results_path, unambiguous=False):
    ...
    with open(report_file, "rt", encoding='latin-1') as file:
        # read file
        ...
        for i, report in enumerate(reports):
            ...
            for j, acr in enumerate(acr_index):
                try:
                    acr_class = remaining[acr + 4]
                except IndexError:
                    continue

                if not acr_class in ['A', 'B', 'C', 'D', 'a', 'b', 'c', 'd']:

                    # try if there is no spacing
                    acr_class = remaining[acr + 3]

                    if not acr_class in ['A', 'B', 'C', 'D', 'a', 'b', 'c', 'd']:

                        # try whether birads class is undefined or not given --> continue
                        if acr_class == '?' or acr_class == '.':
                            continue

                        # try if there is double spacing
                        try:
                            acr_class = remaining[acr + 5]
                            if not acr_class in ['A', 'B', 'C', 'D', 'a', 'b', 'c', 'd']:
                                continue
                        except:
                            continue

                acr_classes.append(acr_class)

            # more than once ACR in text?
            if len(acr_classes) > 1:
                # all the same?
                if all_equal(acr_classes):
                    acr_class_final = acr_classes[0]
                else:
                    acr_class_final = None

            # zero valid birads classes ? continue!
            elif len(acr_classes) == 0:
                acr_class_final = None

            # only one acr class? GREAT!
            else:
                acr_class_final = acr_classes[0]
        ...
        for date in extracted_dict['birth_date']:
            date = date.split('.')
            final_dict['birth_day'].append(int(date[0]))
            final_dict['birth_month'].append(int(date[1]))
            final_dict['birth_year'].append(int(date[2]))
        ...
        for file in files:
            ...
            for i, (patient_id, day, month, year, birads, acr, birth_year) in enumerate(zip(df['KIS'], df['day'],
                ...
                if int(KIS) == patient_id and study_date == date:
                    ...
                    matching_dict['session_nb'].append(nb_session)
                    nb_matches += 1
